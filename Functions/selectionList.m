...
function [list] = selectionList(list,files,opt)
...
dy=[butH+2*dd,0,titH+2*dd]; %lists y-spaces: bottom, between, top
%calculate positions
...
pos.title = [pos.lists(1,1),dy(1)+lisH+dd,pos.figure(3)-dy(3),titH];
...
gui.h.title=uicontrol('parent',gui.h.figure,...
    'style','text',...
    'unit','pixel','position',pos.title,...
    'fontsize',18,'fontweight','bold','string',titleMain);
